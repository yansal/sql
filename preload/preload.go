...
func WithCache(c Cache) func(*Options) { return func(o *Options) { o.cache = c } }

// Struct preloads fields into dest, which must be a pointer to a struct.
func Struct(ctx context.Context, db Querier, dest interface{}, fields []Field, opts ...Option) error {
	ptrvalue := reflect.ValueOf(dest)
	if kind := ptrvalue.Kind(); kind != reflect.Ptr {
		panic(fmt.Sprintf("preload: dest is a value of kind %s, must be pointer to a struct", kind))
	}
	if kind := ptrvalue.Type().Elem().Kind(); kind != reflect.Struct {
		panic(fmt.Sprintf("preload: dest is a pointer to a value of kind %s, must be pointer to a struct", kind))
	}
	if ptrvalue.IsNil() {
	...
	return preloadall(ctx, db, slicevalue, fields, opts...)
}

// StructSlice preloads fields into dest, which must be a slice of structs.
func StructSlice(ctx context.Context, db Querier, dest interface{}, fields []Field, opts ...Option) error {
	destvalue := reflect.ValueOf(dest)
	if kind := destvalue.Kind(); kind != reflect.Slice {
		panic(fmt.Sprintf("preload: dest is a value of kind %s, must be slice of structs", kind))
	}
	elemtype := destvalue.Type().Elem()
	if kind := elemtype.Kind(); kind != reflect.Struct {
		panic(fmt.Sprintf("preload: dest is a slice of values of kind %s, must be slice of structs", kind))
	}
	if destvalue.Len() == 0 {
		return nil
	}
...
func preloadall(ctx context.Context, db Querier, slicevalue reflect.Value, fields []Field, opts ...Option) error {
	var o Options
	for i := range opts {
		opts[i](&o)
	}
	for _, field := range fields {
		if err := preload(ctx, db, slicevalue, field, &o); err != nil {
			return err
		}
	}
	return nil
...
func preload(ctx context.Context, db Querier, slicevalue reflect.Value, field Field, opts *Options) error {
	...
	if field.Cached && opts.cache != nil {
		cachefields := make([]string, 0, len(query.bindvalues))
		for i := range query.bindvalues {
			cachefields = append(cachefields, fmt.Sprint(query.bindvalues[i]))
		}
		_, err := opts.cache.Get(ctx, query.table, cachefields...)
		if err != nil {
			return err
			// TODO: log err and fallback to query?
		}
		// TODO: set parent to values
	}
	...
	// 	for i := range res.destscantagvalues {
	// 		key := fmt.Sprint(res.destscantagvalues[i])
	// 		binarymarshaler, ok := res.destvalues[i].Interface().(encoding.BinaryMarshaler)
	// 		if !ok {
	// 			panic(fmt.Sprintf("preload: %s must implement encoding.BinaryMarshaler", res.destvalues[i].Type()))
	// 		}
	...
	// 		values[key] = value
	// 	}
	// 	if err := opts.cache.Set(ctx, strings.Join([]string{query.table, query.whereident}, "."), values); err != nil {
	// 		// TODO: silent fail?
	// 		return err
	// 	}
	// }
...
func (q *query) do(ctx context.Context, db Querier) (*results, error) {
	...
	for {
		...
		s := build.Select(build.Columns(q.columns...)...).
