...
func TestSelect(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		stmt: Select(Columns("foo")...).
			From(Ident("table")).
			Where(Ident("created_at").LessThan(Bind(now))),
		out:  `SELECT "foo" FROM "table" WHERE "created_at" < $1`,
		args: []interface{}{now},
	}, {
		stmt: Select(Columns("foo", "bar")...).From(Ident("table")).
			Where(Ident("foo").In(Bind([]string{"hello", "world"}))),
		out:  `SELECT "foo", "bar" FROM "table" WHERE "foo" IN ($1, $2)`,
		args: []interface{}{"hello", "world"},
	}, {
		stmt: Select(Columns("foo")...).From(Ident("bar")).
			Where(Ident("foo").IsNull()),
		out: `SELECT "foo" FROM "bar" WHERE "foo" IS NULL`,
	}, {
		stmt: Select(CallExpr("foo", Bind("hello"), Int(123))),
		out:  `SELECT foo($1, 123)`,
		args: []interface{}{"hello"},
	}, {
		stmt: Select(Columns("foo")...).From(Ident("bar")).Limit(Int(1)).Offset(Int(2)),
		out:  `SELECT "foo" FROM "bar" LIMIT 1 OFFSET 2`,
	}, {
		stmt: Select(Columns("foo")...).From(Ident("bar")).Limit(Bind(1)).Offset(Bind(2)),
		out:  `SELECT "foo" FROM "bar" LIMIT $1 OFFSET $2`,
		args: []interface{}{1, 2},
	}, {
		stmt: Select(CallExpr("count", Star), Ident("foo")).From(Ident("bar")).GroupBy(Ident("foo")),
		out:  `SELECT count(*), "foo" FROM "bar" GROUP BY "foo"`,
	}, {
		stmt: Select(Columns("now")...).From(
		...
		stmt: Select(Columns("foo")...).From(Ident("bar")).OrderBy(Ident("foo")),
		...
		stmt: Select(Columns("location", "time", "report")...).
		...
		stmt: Select(Ident("distributors.name")).
